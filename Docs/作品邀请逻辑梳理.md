...
- 只有“被邀请用户在该作品上的首次签到”才会建立邀请关系。
- 非首次签到即使携带邀请码，也不会建立新的邀请关系。
...
### 2.4 邀请弹窗数据来源拆分（组队 vs 邀请列表）

`apps/web/src/components/Modals/WorkDetailModals.tsx` 现已按两个独立接口展示：

1. 组队区域（3-Person Team）
- 数据来源：`GET /arts/work/detail`
- 字段：`has_team`、`team_members`、`team_sign_in_progress`、`team_sign_in_reward_claimed`
- 说明：该区域不再复用 `work/friendsList` 数据拼装。

2. 邀请列表区域（Invitation List）
- 数据来源：`GET /arts/work/friendsList`
- 字段：`invite`、`email`、`wallet_address`、`wallet_display`、`invitation_time`、`signed_in`
- 说明：继续使用邀请列表接口，不与组队成员接口混用。
