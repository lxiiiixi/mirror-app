...
function App() {
    ...
    return (
        <AppLayout
            showWalletBar={!isAssetsPage}
            showPageNav={isAssetsPage}
            languageLabel={t("header.language")}
            assetsLabel={t("header.assets")}
            loginLabel={t("header.login")}
            isLoggedIn={isLoggedIn}
            pageTitle={isAssetsPage ? t("assets.title") : undefined}
            backIconSrc={isAssetsPage ? images.works.backBtn : undefined}
            onBack={isAssetsPage ? () => navigate(-1) : undefined}
            onLanguageClick={handleLanguageToggle}
            onLogoClick={() => navigate("/")}
            onWalletClick={handleWalletClick}
            footerItems={footerItems}
            activeFooterIndex={activeFooterIndex}
            showFooter={!isAssetsPage}
        >
            <Routes>
