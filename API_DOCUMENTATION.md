...
- [7. 票券模块 (Ticket)](#7-票券模块-ticket)
- [8. 寄售模块 (Consignment)](#8-寄售模块-consignment)
- [9. 渠道商模块 (Channel)](#9-渠道商模块-channel)
- [10. 健康检查模块 (Health)](#10-健康检查模块-health)
- [11. 管理员模块 (Admin)](#11-管理员模块-admin)
...
获取作品的外部链接列表。当该作品未配置任何外部链接时，`data` 为空数组 `[]`。
...
| Header | 类型 | 必填 | 说明 |
|--------|------|------|------|
| token | string | 否 | 用户token（可选，CheckTokenNotPrompt） |
  ...
  "data": [
    {
      ...
      "link_url": "https://example.com",
      "create_time": "2026-01-26T10:00:00+08:00",
      "update_time": "2026-01-26T10:00:00+08:00"
    }
  ]
}
```
...
**说明**：`data` 为数组，无记录时返回 `[]`。外部链接通过「上传外部链接」接口写入。
...
获取作品关联的链接列表（章节/内容列表）。支持分页，兼容老接口（不传分页参数时返回全量数组）。
...
**请求参数**
```
work_id=214&page=1&page_size=15
```

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| work_id | int | 是 | 作品ID |
| page | int | 否 | 页码（从1开始），不传则返回全量数组（兼容模式） |
| page_size | int | 否 | 每页数量（默认15），不传则返回全量数组（兼容模式） |
...
| chapter_id | int | 章节ID |
| parent_id | int | 作品ID |
| content_type | int | 内容类型（电影：1=预告，2=剧照；其他类型：无此字段） |
| video_url | string | 预告视频URL（仅预告类型） |
| duration_seconds | int | 视频时长（秒，仅预告类型） |
| cover_url | string | 封面/剧照图URL |
| track_number | int | 曲目序号（仅音乐类型） |
| title | string | 标题 |
| content | string | 内容描述 |
| create_time | string | 创建时间 |
| update_time | string | 更新时间 |
...
获取用户邀请的好友列表（被邀请人信息）。返回被邀请人的用户名、脱敏钱包地址、邀请时间、当日签到状态。
...
| 字段 | 类型 | 说明 |
|------|------|------|
| invite | string | 被邀请人的用户名（email或昵称） |
| invitation_time | string | 邀请时间（格式：MM/DD/YYYY） |
| wallet_display | string | 被邀请人的脱敏钱包地址（前3位+***+后2位） |
| signed_in | bool | 被邀请人今日是否已签到（该作品下） |
| total | int | 总邀请人数 |
    ...
    "list": [
      {
        ...
        "update_time": "2026-01-29T14:05:43+08:00"
      }
    ],
    "total": 4
  }
...
```json
{
  ...
  "receiver_region": "北京市 朝阳区",
  "receiver_address": "测试街道123号"
}
```
...
| 错误码 | 说明 |
|--------|------|
| 503 | 库存不足（insufficient stock） |
| 503 | 积分不足（insufficient points） |
| 412 | 参数错误（quantity <= 0） |
    ...
    "list": [
      {
        ...
        "express_no": "",
        "ship_time": null,
        "create_time": "2026-01-29T14:06:05+08:00",
        "update_time": "2026-01-29T14:06:05+08:00"
      }
    ],
    "total": 1
  }
...
| quantity | int | 兑换数量 |
| status | int | 订单状态（0=待发货，1=已发货，2=已完成，3=已取消） |
...
| 错误码 | 说明 |
|--------|------|
| 412 | 参数错误（订单不存在或不属于当前用户） |

---

### 3.7 更新订单收货信息

更新订单的收货地址信息。仅允许在订单状态为"待发货"（status=0）时更新。

**接口地址**
```
POST /arts/points/order/address
```
...
| 错误码 | 说明 |
|--------|------|
| 412 | 参数错误（订单不存在、不属于当前用户或已发货） |

**业务逻辑**
1. 检查订单是否存在且属于当前用户
2. 检查订单状态是否为"待发货"（status=0）
3. 更新收货信息
...
```json
{
  ...
  "data": {
    ...
    "order_id": 17683558763961652
  }
}
```

**说明**：前端付款成功后应轮询「查询支付结果」接口（传 `order_id` 或 `signature`），当返回 `paid: true` 或 `status_text: "paid"` 时跳转支付成功页，避免一直卡在链上确认页。
...
查询会员/节点支付结果，用于付款成功后轮询，当 `paid` 为 true 时前端应跳转支付成功页。
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| order_id | int64 | 二选一 | 订单ID（Send 响应中返回） |
| signature | string | 二选一 | 交易签名（Send 响应中 tx_signature） |
...
| 字段 | 类型 | 说明 |
|------|------|------|
| status | int | 0=未付款 1=已付款 2=处理中 -1=已取消 |
| paid | bool | 为 true 时应跳转支付成功页 |
| status_text | string | pending / paid / processing / cancelled |
| order_id | int64 | 订单ID（按 signature 查询时也会返回） |
